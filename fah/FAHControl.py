...
class FAHControl(SingleAppServer):
    ...
    def __init__(self, glade = 'FAHControl.glade'): #pco init fahcontrol
        SingleAppServer.__init__(self) #pco call to singleappserver
        ...
        print 'pco - load clients'
        self.load_clients()
        print 'pco - clients loaded'

        # If we don't have any clients add the default client
        #pco default client
        if not len(self.clients):
            print 'pco - create default client'
            client = Client(self, 'local', '127.0.0.1', 36330, '')
            #self.client_list = self.client_list.append(client)
            self.add_client(client)
            self.save_clients()
            print 'pco - default client created'
    ...
    def check_clients(self):
        ...
        # Update clients
        for client in self.clients.values(): 
            client.update(self) #pco dict.values()
            if client.is_online(): #pco is_online?
        ...
        if self.active_client and self.active_client.is_online(): #pco is_online?
            #self.client_notebook.set_sensitive(True)
            print 'client active and online'
        else: #self.client_notebook.set_sensitive(False)
            print 'client not active and online'
    ...
    def load_clients(self):
        ...
        for row in self.db.select('clients', orderby = 'name'):
            ...
            print 'client:' + client.name + ',' + client.password + ',' + client.address + ':' + str(client.port)
